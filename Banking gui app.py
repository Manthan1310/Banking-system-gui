...
#Main Screen
master = Tk()   #Tk to initialize screen 
...
def register():
    ...
    #labels
    Label(register_screen,text="Please enter your details below to register ",font=('calibiri',12)).grid(row=0,sticky=N,pady=10)
    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                #Buttons
                Button(account_dashboard , text="Personal Details",font=('Calibiri',12),width=30,command=personal_details).grid(row=2,sticky=N,pady=10)
                Button(account_dashboard , text="Deposit",font=('Calibiri',12),width=30,command=deposit).grid(row=3,sticky=N,padx=10)
                Button(account_dashboard , text="Withdraw",font=('Calibiri',12),width=30,command=withdraw).grid(row=4,sticky=N,pady=10)
...
def deposit():
    ...
    current_balance_label = Label(deposit_screen , text="Current Balance : $"+details_balance , font=('Calibiri',12))
...
def finish_deposit():
    ...
    if float(amount.get())<=0:
        deposit_notif.config(text='Negative currency is not repeated ', fg="red")
        return
...
def withdraw():
    ...
    current_balance_label = Label(withdraw_screen , text="Current Balance : $"+details_balance , font=('Calibiri',12))
    current_balance_label.grid(row=1,sticky=W)
    ...
    #Button
    Button(withdraw_screen,text="Finish",font=('Calibri',12),command=withdraw_deposit).grid(row=3,sticky=W,pady=5)
def withdraw_deposit():
    if withdraw_amount.get() == "":
        withdraw_notif.config(text='Amount is required !',fg="red")
        return
    if float(withdraw_amount.get())<=0:
        withdraw_notif.config(text='Negative currency is not repeated ', fg="red")
        return
    ...
    if float(withdraw_amount.get()) > float(current_balance):
        withdraw_notif.config(text='Insufficiante Funds ! ',fg='red')
        return
...
def personal_details():
    ...
    #Labels
    Label(personal_details_screen , text='Personal Details ' , font=('Calibiri',12)).grid(row=0,sticky=N,pady=10)
    Label(personal_details_screen , text='Name :'+details_name , font=('Calibiri',12)).grid(row=1,sticky=N,pady=10)
    Label(personal_details_screen , text='Age :'+details_age , font=('Calibiri',12)).grid(row=2,sticky=N,pady=10)
    Label(personal_details_screen , text='Gender:'+details_gender , font=('Calibiri',12)).grid(row=3,sticky=N,pady=10)
    Label(personal_details_screen , text='Balance :$'+details_balance , font=('Calibiri',12)).grid(row=4,sticky=N,pady=10)
def login():
    #variables
    ...
    #Labels
    Label(login_screen, text="Login to yout account " , font=('Calibiri',12)).grid(row=0,sticky=N,pady=10)
    Label(login_screen, text="Username  " , font=('Calibiri',12)).grid(row=1,sticky=N,pady=10)
    Label(login_screen, text="Password " , font=('Calibiri',12)).grid(row=2,sticky=N,pady=10)
    ...
    #Button
    Button(login_screen , text=" Login ", command=login_session , width=15 , font=('Calibiri',12)).grid(row=3,sticky = W,padx=5)
...
#Labels   just lets you put text on screen 
